...
export async function getResolvedEditorWorkspaceBackgroundForRead() {
  let files = []
  try {
  ...
  const hit = files.find((f) => f.startsWith('editor-workspace-background.'))
  if (!hit) return null
...
export async function writeEditorWorkspaceBackground(buffer, ext) {
  await fs.mkdir(UPLOADS_DIR, { recursive: true })
  const e = sanitizeStoredFileExt(ext)
  const target = path.join(UPLOADS_DIR, `editor-workspace-background.${e}`)
  let files = []
  try {
    files = await fs.readdir(UPLOADS_DIR)
  } catch {
  ...
  for (const f of files) {
    if (!f.startsWith('editor-workspace-background.')) continue
...
export async function deleteEditorWorkspaceBackground() {
  let files = []
  try {
    files = await fs.readdir(UPLOADS_DIR)
  } catch {
