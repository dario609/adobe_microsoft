...
export async function getResolvedSessionHeaderBackgroundForRead() {
  let files = []
  try {
  ...
  const hit = files.find((f) => f.startsWith('session-header-background.'))
  if (!hit) return null
...
export async function writeSessionHeaderBackground(buffer, ext) {
  await fs.mkdir(UPLOADS_DIR, { recursive: true })
  const e = sanitizeStoredFileExt(ext)
  const target = path.join(UPLOADS_DIR, `session-header-background.${e}`)
  let files = []
  try {
    files = await fs.readdir(UPLOADS_DIR)
  } catch {
  ...
  for (const f of files) {
    if (!f.startsWith('session-header-background.')) continue
...
export async function deleteSessionHeaderBackground() {
  let files = []
  try {
    files = await fs.readdir(UPLOADS_DIR)
  } catch {
